...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    private func setupWindow() {
        let initialVC = CategoriesViewController()
    ...
    private func setupNotifications() {
        NotificationCenter.default.addObserver(self, selector: #selector(showNoInternetMessage), name: .InternetNotAvailable, object: nil)
    }
    
    @objc private func showNoInternetMessage() {
        guard let currentVC = UIApplication.topViewController(), currentVC is UIAlertController == false else { return }
        HUD.hide()
